...
class Store:
    ...
    Thread safety: every public method acquires ``self._lock`` (an
    ``RLock``) before touching ``self._conn``, making it safe to call
    from multiple threads (e.g. API request threads and background
    ingestion workers) without data corruption or SQLite "database is
    locked" errors.
    """
    ...
    def fact_count(self, doc_id: str) -> int:
        """Return the number of facts stored for a given document id."""
        with self._lock:
            row = self._conn.execute(
